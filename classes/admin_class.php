...
/* -------------admin_password_change------------*/
	...
	public function add_new_task($data){
		...
		$t_weeks = $this->test_form_input_data($data['t_weeks']);
		$t_days = $this->test_form_input_data($data['t_days']);
		$date = date('Y-m-d');
		

		try{
			$add_task = $this->db->prepare("INSERT INTO task_info (t_title, t_description, t_weeks, t_days, t_user_id, date) VALUES (:x, :y, :z, :a, :b, :d) ");

			$add_task->bindparam(':x', $task_title);
			$add_task->bindparam(':y', $task_description);
			$add_task->bindparam(':z', $t_weeks);
			$add_task->bindparam(':a', $t_days);
			$add_task->bindparam(':b', $_SESSION['admin_id']);
			$add_task->bindparam(':d', $date);
		...
		public function update_task_info($data, $task_id, $user_role){
			...
			$t_weeks = $this->test_form_input_data($data['t_weeks']);
			$t_days = $this->test_form_input_data($data['t_days']);
			$evaluation = $this->test_form_input_data($data['evaluation']);
 ...
 */
			try{
				$update_task = $this->db->prepare("UPDATE task_info SET t_title = :x, t_description = :y, t_weeks = :z, t_days = :a, t_user_id = :b, evaluation = :c WHERE task_id = :id ");

				$update_task->bindparam(':x', $task_title);
				$update_task->bindparam(':y', $task_description);
				$update_task->bindparam(':z', $t_weeks);
				$update_task->bindparam(':a', $t_days);
				$update_task->bindparam(':b', $_SESSION['admin_id']);
				$update_task->bindparam(':c', $evaluation);
